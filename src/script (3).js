/*define(['https://api.gosklad.ru/script.js?v=' + (new Date().getTime())], function (CustomWidget) {
    return CustomWidget;
});*/
  ...
  return function () {
    ...
    self.warehouseRender = function () {
      ...
      self
        ...
        .done(function (result) {
          if (result.token) {
            $("#work-area-" + settings.widget_code).attr(
              "style",
              "width: 100%; height: 1400px; max-height: calc(100vh - 130px);"
            );
            $("#work-area-" + settings.widget_code).html(
              '<iframe  allow="clipboard-read; clipboard-write" token="' +
                result.token +
                '" name="gosklad" style="width: 100%; height: 100%;" src="https://app.gosklad.ru?token=' +
                result.token +
                '" ' +
        ...
        .done(function (result) {
          if (result.success) {
            if (productsBox.length === 0) {
              var fieldProducts = $(
                '[data-id="' + result.data.field_products + '"]'
              );
              productsBox = $(
                '<div style="position: relative;" id="products_box"></div>'
              );
            ...
            html.find(".gs_field_price_type").on("change", function () {
              ...
              let priceFields = $(this)
                .parents(".sls-card-item")
                .find(".gs_field_price");
              if (current === newValue) {
                priceFields.show();
              } else {
    ...
    self.initProductsTabEvents = function (leadId) {
      $(document).on(
        "change",
        ".catalog-fields__container-item--amount input",
        function () {
      ...
      $(document).on(
        "DOMSubtreeModified",
        ".linked-form-holder.js-cf-group-wrapper.catalog_elements-in_card.catalog_products-in_card",
        function () {
    ...
    self.hideAmoProductsTab = function (leadId) {
      let catalogTab = $(
        '.card-tabs__item[data-id="' + self.amo_products_tab_id + '"]'
      );
      if (catalogTab.length > 0) {
        catalogTab.hide();
      } else {
        let productsTabId = $(
          '.js-card-tab .card-tabs__item-inner:contains("Товары")'
        )
          .parent(".js-card-tab")
          .data("id");
        ...
        if (productsTabId !== undefined) {
          if (
            AMOCRM.data.current_card.tabs._selected_tab === productsTabId &&
            !self.isInitProductsTab
          ) {
    ...
    self.apiRequest = function (url, data, callback) {
      let _data = {
        account_id: AMOCRM.constant("account").id,
        user: AMOCRM.constant("user").id,
    ...
    self.lockProductsTab = function () {
      ...
      let strLock =
        '<div class="slsUpProductsLock" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; z-index: 10;"></div>';
    ...
    self.showProductsPreloader = function (isFirst) {
      ...
      let whBoxFirst = $('[data-id="' + self.products_tab_id + '"]').parent(
        "div"
      );

      let strPreloader =
        '<div class="slsUpProductsPreloader" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; z-index: 10;"></div>';
      let strBg =
        '<div style="position: absolute; width: 108%; height: 102%; top: -1%; left: -4%; z-index: 10; background-color: gray; opacity: 0.2;"></div>';
      let imgSrc = "https://warehouse2.salesup.pro/images/spinner.gif";
      let strImage =
        '<img style="position: absolute; top: 50%; left: 50%; margin: -35px 0 0 -35px;" src="' +
        imgSrc +
        '">';
    ...
    self.showProductsSearch = function (callback) {
      ...
      self
        ...
        .done(function (result) {
          ...
          productsBox = $(
            '<div style="position: relative;" id="products_box_search"></div>'
          );
          fieldProducts.after(productsBox);
          fieldProducts.hide();
    ...
    this.callbacks = {
      settings: function ($modal_body) {
        ...
        sett.after(
          "<div class='support'>" +
            ...
            "<div class='s_icons'>" +
            "<div class='s_icon'><a href='https://t.me/goskladbot' target='_blank'><img src=" +
            settings.path +
            "/images/tg.png?v=" +
            settings.version +
            "/></a></div>" +
            "<div class='s_icon'><a href='https://wa.me/74954199745' target='_blank'><img src=" +
            settings.path +
            "/images/wa.png?v=" +
            settings.version +
            "/></a></div>" +
      ...
      render: function () {
        ...
        if (AMOCRM.isCard() && AMOCRM.getBaseEntity() == "leads") {
          ...
          $("head").append(
            '<link type="text/css" rel="stylesheet" href="https://ajax.googleapis.com/ajax/libs/jqueryui/1.12.1/themes/smoothness/jquery-ui.css?v=' +
              Date.now() +
              '" >'
          );
